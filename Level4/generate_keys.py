...
def generate_keys(alpha_1,alpha_2,gamma, index):
    alpha=int(alpha_1,2)^int(alpha_2,2)
    gamma=int(gamma,2)
    beta_1=0
    beta_2=0
    keys=[]
    for beta_1 in range(64):
        beta_2=alpha^beta_1
        gamma_1=S[index][beta_1-1]
        gamma_2=S[index][beta_2-1]
        if gamma_1^gamma_2==gamma:
            k=beta_1^int(alpha_1,2)
            k=format(k,'0>6b')
            keys.append(k)
    return keys
...
def main():
    ...
    for i in range(len(expansion_output)):
        for j in range(0,8):
            a=generate_keys(expansion_output[i][0][6*j:6*j+6], expansion_output[i][1][6*j:6*j+6], S_box_output[i][4*j:4*j+4], j)
            # if len(a)!=0:
            keys.append(a)
            # else:
            #     keys.append('\n')
